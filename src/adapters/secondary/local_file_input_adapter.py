...
プロトタイプ用のローカルファイルベースの入力データソース。
test_inputディレクトリ構造に対応。
"""
import json
import logging
...
class LocalFileInputAdapter(IInputDataSource):
    """
    ローカルファイルからの入力データアダプター
    
    注意: これはプロトタイプ用の実装です。
    本番環境では異なる入力ソースに置き換わる可能性があります。
    """
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        Args:
            config: 設定辞書
                - base_path: ベースディレクトリパス
                - video_file: 動画ファイル名（オプション）
                - detections_file: 検出情報JSONファイル名（オプション）
                - mask_directory: マスク画像ディレクトリ名（オプション）
        """
        # ベースパス設定
        ...
        logger.info(f"LocalFileInputAdapter initialized with base_path: {self.base_path}")
    ...
    def get_mask(self, frame_index: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            object_ids = [int(id_val) for id_val in unique_ids if id_val > 0]
            
            # バイナリマスク（値が255のみ）の場合の処理
            if len(unique_ids) == 2 and 255 in unique_ids:
                ...
                if detections:
                    ...
                    mask_data = np.where(mask_data > 0, track_id + 1, 0).astype(np.uint8)
                    
                    # オブジェクトIDリスト
                    object_ids = [track_id + 1]
                    ...
                    mask_data = np.where(mask_data > 0, 1, 0).astype(np.uint8)
                    object_ids = [1]
                    classes = {1: "unknown"}
                    confidences = {1: 1.0}
            else:
                # 通常のマスク（複数のオブジェクトID）の場合
                detections = self.get_detections(frame_index)
                ...
                for det_dict in detections:
                    ...
                    if track_id in object_ids:
                        classes[track_id] = det_dict["class"]["name"]
                        confidences[track_id] = det_dict["confidence"]
    ...
    def close(self) -> None:
        ...
        self._is_initialized = False
        logger.info("LocalFileInputAdapter closed")